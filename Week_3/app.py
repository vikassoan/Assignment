...
with st.container():
    st.markdown("### 📊 EV Adoption Dashboard")
    st.markdown("This dashboard helps you explore and forecast Electric Vehicle (EV) adoption trends by county in Washington State.")
...
# Instruction line
st.subheader("Select a county to forecast EV adoption for the next 3 years.")
...
fig, ax = plt.subplots(figsize=(12, 6), facecolor='white')  # set full figure bg to white
ax.set_facecolor('white')  # set inner plot bg to white

# Plot lines with visible color and dots
for label, data in combined.groupby('Source'):
    ax.plot(data['Date'], data['Cumulative EV'], label=label, marker='o', linewidth=2)

# Titles and labels with dark color
ax.set_title(f"Cumulative EV Forecast - {county}", fontsize=16, color='#111')
ax.set_xlabel("Date", fontsize=12, color='#333')
ax.set_ylabel("Cumulative EV Count", fontsize=12, color='#333')

# Tick labels with dark color
